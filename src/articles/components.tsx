      ...
      <div className="flex items-center gap-3 pb-6 border-b border-border">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
            ...
            <span className="inline-flex items-center gap-1"><Clock className="w-3.5 h-3.5" aria-hidden="true" />{readingTime}</span>
  ...
  return (
    <footer className="mt-16 pt-8 border-t border-border">
      ...
      <div className="flex gap-3 mb-8">
        ...
        <a href={site.social.linkedin} target="_blank" rel="me noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#0A66C2]/10 border border-[#0A66C2]/20 text-sm font-medium text-[#0A66C2] hover:bg-[#0A66C2]/20 transition-colors">
        ...
        {site.social.github && (
        <a href={site.social.github} target="_blank" rel="me noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-card border border-border text-sm font-medium text-muted-foreground hover:text-foreground hover:border-primary/30 transition-colors">
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>&copy; {new Date().getFullYear()} {site.fullName}. {f.copyright}</span>
        <span className="text-border">|</span>
        <Link to={lang === 'es' ? '/privacidad' : '/privacy'} className="underline underline-offset-2 hover:text-primary transition-colors">
...
export function InlineWorkflowDownload({ href, label, fileSize }: { href: string; label: string; fileSize: string }) {
  ...
  return (
    ...
    >
      {isExternal ? <ExternalLink className="w-3.5 h-3.5" aria-hidden="true" /> : <Download className="w-3.5 h-3.5" aria-hidden="true" />}
      {label}
...
export function WorkflowDownloadCard({ icon, name, subtitle, description, href, fileSize, nodes, llm, downloadLabel }: WorkflowDownloadCardProps) {
  return (
    <div className="bg-card border border-border rounded-lg p-5 flex flex-col">
      ...
      >
        {href.startsWith('http') ? <ExternalLink className="w-4 h-4 text-primary" aria-hidden="true" /> : <Download className="w-4 h-4 text-primary" aria-hidden="true" />}
        {downloadLabel}
      </a>
    </div>
  )
